...
class VideoList(AssetsTable):
    ...
    def __init__(self, parent):
        ...
        self.setHorizontalHeaderLabels(['Name', 'Alias', 'Size'])
        self.setResizeMode([QH.ResizeToContents, QH.Stretch, QH.ResizeToContents])
    ...
    def create_list(self, files):
        if files:
            ...
            for i, file in enumerate(files):
                name_item = QTableWidgetItem(os.path.basename(file).rstrip(".usm"))
                name_item.setIcon(FIF.VIDEO.icon())
    ...
    def getRealIndexInFileList(self, name):
        for index, file in enumerate(self.files):
            if os.path.basename(file).rstrip(".usm") == name:
                return index, file
